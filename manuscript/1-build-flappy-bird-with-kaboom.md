...
Get the *Flappy Bird* assets [here](https://docs.replit.com/tutorial-files/flappy-bird-kaboom/flappy-assets.zip).

Let's import the game assets (graphics and sound). We can use Kaboom's `loadSprite` and `loadSound` functions:
...
Kaboom has an `origin` component that sets the point a character uses as its origin. This is `topleft` by default, which works well for our lower pipe, as our calculations above are calculating for that point. However, for the upper pipe, our calculations are for the _bottom_ of the pipe. Therefore, we can use the `origin` component to specify that. 
